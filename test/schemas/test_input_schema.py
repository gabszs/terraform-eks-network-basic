...
def test_invalid_char_inject():
    with pytest.raises(ValidationError, match="Invalid Char Regex"):
        PasswordBody(adicional_lenght=10, quantity=5, char_inject=["abc", "d"], string_inject=["valid_string"])


def test_invalid_string_inject():
    with pytest.raises(ValidationError, match="Invalide String Regex"):
        PasswordBody(adicional_lenght=10, quantity=5, char_inject=["s", "b"], string_inject=["a"])


def test_invalid_adicional_lenght():
    with pytest.raises(ValidationError):
        PasswordBody(adicional_lenght=-1, quantity=5, char_inject=["s"], string_inject=["valid_string"])
